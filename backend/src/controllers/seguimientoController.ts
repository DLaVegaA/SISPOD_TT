...
export const crearSeguimiento = async (req: CustomRequest, res: Response, next: NextFunction) => {
  ...
  const id_cuestionario_24h = req.body.id_cuestionario_24h
    ? Number(req.body.id_cuestionario_24h)
    : null;
  const id_cuestionario_72h = req.body.id_cuestionario_72h
    ? Number(req.body.id_cuestionario_72h)
    : null;
...
export const editarSeguimiento = async (req: CustomRequest, res: Response, next: NextFunction) => {
  const id_seguimiento = Number(req.params.id_seguimiento);
  const { plan_cuidados, indicaciones_medicas, id_cuestionario_24h, id_cuestionario_72h } =
    req.body;
...
export const cancelarSeguimiento = async (
  req: CustomRequest,
  res: Response,
  next: NextFunction,
) => {
...
export const finalizarSeguimiento = async (
  req: CustomRequest,
  res: Response,
  next: NextFunction,
) => {
...
export const obtenerCuestionarioSeguimiento = async (
  req: CustomRequest,
  res: Response,
  next: NextFunction,
) => {
  ...
  try {
    ...
    const cuestionario = await obtenerCuestionarioSeguimientoService(
      id_seguimiento,
      tipo_cuestionario,
    );
...
// ─── Obtener respuestas del seguimiento ───────────────────────────────────────

export const obtenerRespuestasSeguimiento = async (
  req: CustomRequest,
  res: Response,
  next: NextFunction,
) => {
...
export const resolverAlerta = async (req: CustomRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    return res.json({
      message: 'Alerta resuelta. El seguimiento regresó a "en curso".',
      seguimiento,
    });
  } catch (error) {
    next(error);
