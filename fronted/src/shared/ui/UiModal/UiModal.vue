<template>
  <Transition name="modal">
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      />
      <div :class="['relative bg-card border border-border rounded-2xl w-full shadow-2xl slide-in', maxWidthClass]">
        <div v-if="title" class="flex items-center justify-between px-6 py-5 border-b border-border">
          <h2 class="font-display font-bold text-black">{{ title }}</h2>
          <button
            type="button"
