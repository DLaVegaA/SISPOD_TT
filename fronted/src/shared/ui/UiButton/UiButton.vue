<template>
  <button :type="type" :disabled="disabled" :class="[base, variantCls, sizeCls]" v-bind="$attrs">
    <slot />
  </button>
</template>
...
const variantMap: Record<Variant, string> = {
  primary: 'bg-accent hover:bg-accent text-black',
  danger: 'bg-accent hover:bg-accent text-black',
  ghost: 'text-muted hover:text-black hover:bg-ghost',
  outline: 'border border-border text-muted hover:text-black hover:border-ghost',
}
