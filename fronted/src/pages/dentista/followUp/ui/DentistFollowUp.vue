...
interface Seguimiento {
  id_seguimiento: number; id_cita: number; id_procedimiento: number
  id_cuestionario_24h: number | null; id_cuestionario_72h: number | null
  estado_seguimiento: 'en curso' | 'finalizado' | 'alerta' | 'cancelado'
  plan_cuidados: string; indicaciones_medicas: string
  fecha_inicio: string; fecha_fin: string | null
  pacienteNombre: string; procedimientoNombre: string
  tiene_cuestionario_24h: boolean; tiene_cuestionario_72h: boolean
  enviado_24h: boolean; enviado_72h: boolean
}

interface RespuestaItem {
  id_pregunta_base: number; texto_pregunta: string
  tipo_control: 'escala_1_10' | 'booleano_si_no' | 'texto_libre' | 'opcion_multiple'
  opciones: string[] | null; valor_alerta: Record<string, any> | null
  valor_respuesta: string; fecha_respuesta: string; disparo_alerta: boolean
}

interface RespuestasDetalle {
  respuestas_24h: RespuestaItem[]; respuestas_72h: RespuestaItem[]
  fecha_envio_24h: string | null; fecha_envio_72h: string | null
}
...
const form = ref({
  id_seguimiento: 0, id_cita: '' as string, id_procedimiento: '' as string,
  plan_cuidados: '', indicaciones_medicas: '',
  id_cuestionario_24h: '' as string, id_cuestionario_72h: '' as string,
})
...
const columns = [
  { key: 'alerta',     label: 'Alerta',     icon: AlertCircle,  accent: 'text-red-500',     borderLeft: 'border-l-red-400',     badge: 'bg-red-500/10 text-red-600 border-red-400/30',           headerBg: 'bg-red-500/5 border-red-400/20' },
  { key: 'en curso',   label: 'En Curso',   icon: Clock,        accent: 'text-accent',      borderLeft: 'border-l-accent',      badge: 'bg-accent/10 text-accent border-accent/20',              headerBg: 'bg-accent/5 border-accent/20' },
  { key: 'finalizado', label: 'Finalizado', icon: CheckCircle2, accent: 'text-emerald-500', borderLeft: 'border-l-emerald-400', badge: 'bg-emerald-500/10 text-emerald-600 border-emerald-400/30', headerBg: 'bg-emerald-500/5 border-emerald-400/20' },
]
...
function getInitials(name: string) { return name.split(' ').slice(0, 2).map(n => n.charAt(0)).join('').toUpperCase() }
function diasTranscurridos(fecha: string) { return Math.floor((Date.now() - new Date(fecha).getTime()) / 86400000) }
...
async function fetchSeguimientos() {
  ...
  try {
    ...
    seguimientos.value = (res.seguimientos ?? []).map((s: any) => ({
      id_seguimiento: s.id_seguimiento, id_cita: s.id_cita ?? 0, id_procedimiento: s.id_procedimiento ?? 0,
      id_cuestionario_24h: s.id_cuestionario_24h ?? null, id_cuestionario_72h: s.id_cuestionario_72h ?? null,
      estado_seguimiento: s.estado_seguimiento, plan_cuidados: s.plan_cuidados ?? '', indicaciones_medicas: s.indicaciones_medicas ?? '',
      fecha_inicio: s.fecha_inicio, fecha_fin: s.fecha_fin,
      pacienteNombre: s.nombre ?? '—', procedimientoNombre: s.procedimiento ?? '—',
  ...
  } catch { errorMsg.value = 'No se pudieron cargar los seguimientos.' }
  finally { isLoading.value = false }
}

function abrirModal(modo: 'crear' | 'editar', item?: Seguimiento) {
  modalMode.value = modo; errorMsg.value = null; itemEditando.value = modo === 'editar' ? (item ?? null) : null
  form.value = modo === 'editar' && item
    ? { id_seguimiento: item.id_seguimiento, id_cita: item.id_cita.toString(), id_procedimiento: item.id_procedimiento.toString(), plan_cuidados: item.plan_cuidados, indicaciones_medicas: item.indicaciones_medicas, id_cuestionario_24h: item.id_cuestionario_24h?.toString() ?? '', id_cuestionario_72h: item.id_cuestionario_72h?.toString() ?? '' }
    : { id_seguimiento: 0, id_cita: '', id_procedimiento: '', plan_cuidados: '', indicaciones_medicas: '', id_cuestionario_24h: '', id_cuestionario_72h: '' }
  isModalOpen.value = true
}
function cerrarModal() { isModalOpen.value = false; errorMsg.value = null; itemEditando.value = null }
...
async function guardarSeguimiento() {
  ...
  try {
    if (modalMode.value === 'crear') {
      await httpClient.post('/seguimiento', { id_cita: Number(form.value.id_cita), id_procedimiento: Number(form.value.id_procedimiento), plan_cuidados: form.value.plan_cuidados || null, indicaciones_medicas: form.value.indicaciones_medicas || null, id_cuestionario_24h: form.value.id_cuestionario_24h ? Number(form.value.id_cuestionario_24h) : null, id_cuestionario_72h: form.value.id_cuestionario_72h ? Number(form.value.id_cuestionario_72h) : null })
      successMsg.value = 'Seguimiento creado correctamente.'
    } else {
      await httpClient.put(`/seguimiento/${form.value.id_seguimiento}`, { plan_cuidados: form.value.plan_cuidados || null, indicaciones_medicas: form.value.indicaciones_medicas || null, id_cuestionario_24h: form.value.id_cuestionario_24h ? Number(form.value.id_cuestionario_24h) : null, id_cuestionario_72h: form.value.id_cuestionario_72h ? Number(form.value.id_cuestionario_72h) : null })
      successMsg.value = 'Seguimiento actualizado correctamente.'
    }
    cerrarModal(); setTimeout(() => { successMsg.value = null }, 3500); await fetchSeguimientos()
  } catch (error: any) { errorMsg.value = error?.response?.data?.message ?? 'Error al guardar el seguimiento.' }
...
async function abrirDetalle(item: Seguimiento) {
  seguimientoDetalle.value = item; respuestasDetalle.value = null; isDetalleOpen.value = true; isLoadingRespuestas.value = true
  ...
  try {
    ...
    respuestasDetalle.value = { respuestas_24h: res.respuestas_24h ?? [], respuestas_72h: res.respuestas_72h ?? [], fecha_envio_24h: res.fecha_envio_24h ?? null, fecha_envio_72h: res.fecha_envio_72h ?? null }
  } catch { respuestasDetalle.value = { respuestas_24h: [], respuestas_72h: [], fecha_envio_24h: null, fecha_envio_72h: null } }
  finally { isLoadingRespuestas.value = false }
}
function cerrarDetalle() { isDetalleOpen.value = false; seguimientoDetalle.value = null; respuestasDetalle.value = null }

function formatearRespuesta(tipoControl: string, valor: string): string {
  if (tipoControl === 'booleano_si_no') return valor === 'true' ? 'Sí' : 'No'
  if (tipoControl === 'opcion_multiple') return valor.split(',').map((v: string) => v.trim()).join(', ')
  return valor
}
function respuestasDeTab(tab: '24h' | '72h') { return !respuestasDetalle.value ? [] : tab === '24h' ? respuestasDetalle.value.respuestas_24h : respuestasDetalle.value.respuestas_72h }
function fechaEnvioDeTab(tab: '24h' | '72h') {
  if (!respuestasDetalle.value) return null
  const f = tab === '24h' ? respuestasDetalle.value.fecha_envio_24h : respuestasDetalle.value.fecha_envio_72h
  return f ? new Date(f).toLocaleString('es-MX', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : null
}
function tabEnviado(tab: '24h' | '72h') { return !seguimientoDetalle.value ? false : tab === '24h' ? seguimientoDetalle.value.enviado_24h : seguimientoDetalle.value.enviado_72h }
function tabTiene(tab: '24h' | '72h')   { return !seguimientoDetalle.value ? false : tab === '24h' ? seguimientoDetalle.value.tiene_cuestionario_24h : seguimientoDetalle.value.tiene_cuestionario_72h }
function hayAlertas(tab: '24h' | '72h') { return respuestasDeTab(tab).some(r => r.disparo_alerta) }

onMounted(async () => { await Promise.all([fetchSeguimientos(), cargarCatalogos()]) })
</script>
...
<template>
  <div class="fade-in max-w-7xl mx-auto pb-10">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
      ...
      <button class="flex items-center gap-2 bg-ink/65 text-text-secondary hover:bg-ink/80 px-4 py-2.5 rounded-2xl text-sm font-medium transition-all hover:scale-105 active:scale-95 self-start md:self-auto" @click="abrirModal('crear')">
    ...
    <div class="relative max-w-md mb-8">
      ...
      <input v-model="searchQuery" type="text" placeholder="Buscar paciente o procedimiento..." class="w-full pl-10 pr-9 py-2.5 bg-card border border-border rounded-2xl text-sm text-black placeholder:text-muted/40 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all shadow-sm" />
      <button v-if="searchQuery" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted/40 hover:text-muted/70" @click="clearSearch"><X class="w-4 h-4" /></button>
    </div>

    <div v-if="isLoading" class="flex flex-col items-center justify-center py-32 gap-3 text-muted/50">
      <Loader2 class="w-10 h-10 animate-spin text-accent/50" /><p class="text-sm font-medium">Cargando seguimientos...</p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
      <div v-for="col in columns" :key="col.key" class="flex flex-col gap-3">
        <div :class="['flex items-center justify-between px-4 py-3 rounded-2xl border', col.headerBg]">
          ...
          <span class="text-xs font-bold bg-card border border-border text-muted px-2.5 py-0.5 rounded-full">{{ columnItems(col.key).length }}</span>
        </div>

        <div v-if="columnItems(col.key).length === 0" class="bg-card border border-dashed border-border rounded-2xl py-12 flex flex-col items-center gap-2 text-muted/30">
        ...
        <div v-for="item in columnItems(col.key)" :key="item.id_seguimiento"
          ...
          <div class="p-4 space-y-3">
            <div class="flex items-start justify-between gap-2">
              <div class="flex items-center gap-2.5">
                <div class="w-8 h-8 rounded-xl bg-accent/10 border border-accent/20 flex items-center justify-center text-accent font-bold text-xs shrink-0">{{ getInitials(item.pacienteNombre) }}</div>
                <div>
            ...
            <div class="flex gap-1.5">
              <span v-if="!item.tiene_cuestionario_24h" class="text-[10px] font-medium px-2 py-0.5 rounded-lg border bg-gray-50 text-gray-400 border-gray-200">24h —</span>
              <span v-else-if="!item.enviado_24h"       class="text-[10px] font-medium px-2 py-0.5 rounded-lg border bg-amber-50 text-amber-600 border-amber-200">24h ⏳</span>
              <span v-else                              class="text-[10px] font-medium px-2 py-0.5 rounded-lg border bg-green-50 text-green-700 border-green-200">24h ✓</span>

              <span v-if="!item.tiene_cuestionario_72h" class="text-[10px] font-medium px-2 py-0.5 rounded-lg border bg-gray-50 text-gray-400 border-gray-200">72h —</span>
              <span v-else-if="!item.enviado_72h"       class="text-[10px] font-medium px-2 py-0.5 rounded-lg border bg-amber-50 text-amber-600 border-amber-200">72h ⏳</span>
              <span v-else                              class="text-[10px] font-medium px-2 py-0.5 rounded-lg border bg-green-50 text-green-700 border-green-200">72h ✓</span>
            </div>
          ...
          <div class="px-4 py-2.5 bg-surface/60 border-t border-border rounded-b-2xl flex items-center justify-between">
            <div class="flex items-center gap-1.5 text-[10px] text-muted">
              ...
              <template v-if="!item.fecha_fin"><span>·</span><span class="font-semibold text-black">{{ diasTranscurridos(item.fecha_inicio) }} días</span></template>
            ...
            <div class="flex items-center gap-0.5 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
              ...
              <template v-if="item.estado_seguimiento !== 'finalizado' && item.estado_seguimiento !== 'cancelado'">
                <button class="p-1.5 rounded-lg text-muted hover:text-accent hover:bg-accent/5 transition-colors" title="Editar" @click="abrirModal('editar', item)">
                  <Pencil class="w-3.5 h-3.5" />
                </button>
                <button class="p-1.5 rounded-lg text-muted hover:text-emerald-600 hover:bg-emerald-500/10 transition-colors" title="Finalizar" @click="terminarSeguimiento(item.id_seguimiento)">
                  <CheckCircle2 class="w-3.5 h-3.5" />
                </button>
              </template>
              <span v-else class="text-[10px] text-muted/50 italic ml-1">{{ item.estado_seguimiento }}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="cerrarModal">
          ...
          <div class="relative bg-card border border-border rounded-3xl shadow-2xl w-full max-w-lg flex flex-col gap-5 max-h-[90dvh] overflow-y-auto p-6">
            <div class="flex items-start justify-between shrink-0">
              <div>
                <h3 class="font-display font-bold text-black text-base">{{ modalMode === 'crear' ? 'Nuevo Seguimiento' : 'Editar Seguimiento' }}</h3>
                <p class="text-xs text-muted mt-0.5">{{ modalMode === 'crear' ? 'Selecciona la cita y asigna los cuestionarios.' : 'Modifica el plan y los cuestionarios asignados.' }}</p>
              </div>
              <button class="p-1.5 rounded-xl hover:bg-surface transition-colors text-muted hover:text-black shrink-0" @click="cerrarModal"><X class="w-4 h-4" /></button>
            </div>
            <div class="space-y-4">
              ...
              <div class="space-y-1.5">
                <label class="text-[10px] font-bold text-muted uppercase tracking-wider px-1">Cuestionario 24h</label>
                <div class="relative">
                  <select v-model="form.id_cuestionario_24h" class="w-full bg-surface border border-border rounded-xl px-4 py-2.5 text-sm text-black focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all appearance-none">
              ...
              <div class="space-y-1.5">
                <label class="text-[10px] font-bold text-muted uppercase tracking-wider px-1">Cuestionario 72h</label>
                <div class="relative">
                  <select v-model="form.id_cuestionario_72h" class="w-full bg-surface border border-border rounded-xl px-4 py-2.5 text-sm text-black focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all appearance-none">
              ...
              <div class="space-y-1.5">
                ...
                <textarea v-model="form.plan_cuidados" rows="3" placeholder="Ej. Dieta blanda, aplicar hielo..." class="w-full bg-surface border border-border rounded-xl px-4 py-2.5 text-sm text-black placeholder:text-muted/40 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all resize-none" />
              </div>
              <div class="space-y-1.5">
            ...
            <div class="flex flex-col gap-2">
              <button :disabled="isSaving" :class="['w-full py-3 rounded-2xl text-sm font-bold transition-all flex items-center justify-center gap-2', !isSaving ? 'bg-accent text-white shadow-lg shadow-accent/20 hover:scale-[1.02] active:scale-95' : 'bg-surface text-muted cursor-not-allowed border border-border']" @click="guardarSeguimiento">
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="isDetalleOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="cerrarDetalle">
          <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="cerrarDetalle" />
          <div class="relative bg-card border border-border rounded-3xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90dvh]">

            <!-- Header -->
            <div class="px-6 py-5 border-b border-border flex items-start justify-between gap-4 shrink-0">
              <div>
                <div class="flex items-center gap-2 mb-1">
                  <p class="font-display font-bold text-black text-lg">{{ seguimientoDetalle?.pacienteNombre }}</p>
                  <span v-if="seguimientoDetalle?.estado_seguimiento === 'alerta'" class="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-lg bg-red-500/10 text-red-600 border border-red-400/30">
                    <AlertTriangle class="w-3 h-3" /> Alerta activa
                  </span>
                </div>
                <p class="text-sm text-muted">{{ seguimientoDetalle?.procedimientoNombre }} · Cita #{{ seguimientoDetalle?.id_cita }}</p>
              </div>
              <button class="p-1.5 rounded-xl hover:bg-surface transition-colors text-muted hover:text-black shrink-0" @click="cerrarDetalle"><X class="w-4 h-4" /></button>
            </div>

            <!-- Cuerpo -->
            <div class="flex-1 overflow-y-auto">
              <!-- Plan e indicaciones -->
              <div v-if="seguimientoDetalle?.plan_cuidados || seguimientoDetalle?.indicaciones_medicas" class="px-6 pt-5 pb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div v-if="seguimientoDetalle?.plan_cuidados" class="bg-surface rounded-2xl border border-border p-4">
                  <p class="text-[10px] font-bold text-muted uppercase tracking-wide mb-2">Plan de cuidados</p>
                  <p class="text-sm text-black leading-relaxed">{{ seguimientoDetalle.plan_cuidados }}</p>
                </div>
                <div v-if="seguimientoDetalle?.indicaciones_medicas" class="bg-surface rounded-2xl border border-border p-4">
                  <p class="text-[10px] font-bold text-muted uppercase tracking-wide mb-2">Indicaciones médicas</p>
                  <p class="text-sm text-black leading-relaxed">{{ seguimientoDetalle.indicaciones_medicas }}</p>
                </div>
              </div>
              ...
              <div class="px-6 pb-6">
                ...
                <div class="flex gap-2 mb-4">
                  <template v-for="tab in (['24h', '72h'] as const)" :key="tab">
                    <button v-if="tabTiene(tab)"
                      :class="['flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold border transition-all',
                        tabActiva === tab ? 'bg-accent text-white border-accent shadow-sm' : 'bg-surface text-muted border-border hover:border-accent/30']"
                      @click="tabActiva = tab">
                      Cuestionario {{ tab }}
                      <span v-if="tabEnviado(tab) && hayAlertas(tab)" class="w-2 h-2 rounded-full bg-red-400 shrink-0" />
                    </button>
                  </template>
                  <!-- Si no tiene ningún cuestionario -->
                  <p v-if="!seguimientoDetalle?.tiene_cuestionario_24h && !seguimientoDetalle?.tiene_cuestionario_72h" class="text-sm text-muted">Este seguimiento no tiene cuestionarios asignados.</p>
                </div>

                <!-- Loading -->
                <div v-if="isLoadingRespuestas" class="flex items-center justify-center py-12 gap-2 text-muted/50">
                  <Loader2 class="w-6 h-6 animate-spin text-accent/50" /><span class="text-sm">Cargando respuestas...</span>
                </div>

                <!-- Sin cuestionario en este tab -->
                <div v-else-if="!tabTiene(tabActiva)" class="flex flex-col items-center gap-2 py-12 text-muted/40">
                  <HeartPulse class="w-8 h-8" />
                  <p class="text-sm">No se asignó cuestionario de {{ tabActiva }} a este seguimiento.</p>
                </div>

                <!-- Pendiente -->
                <div v-else-if="!tabEnviado(tabActiva)" class="flex flex-col items-center gap-2 py-12 text-muted/40">
                  <Clock class="w-8 h-8" />
                  <p class="text-sm font-medium">El paciente aún no ha respondido el cuestionario de {{ tabActiva }}.</p>
                </div>

                <!-- Respuestas -->
                <div v-else class="space-y-3">
                  <p class="text-xs text-muted mb-4">
                    Respondido el <span class="font-semibold text-black">{{ fechaEnvioDeTab(tabActiva) }}</span>
                    <span v-if="hayAlertas(tabActiva)" class="ml-2 text-red-600 font-bold">
                      · {{ respuestasDeTab(tabActiva).filter(r => r.disparo_alerta).length }} signo(s) de alarma detectado(s)
                    </span>
                  </p>

                  <div v-for="(r, idx) in respuestasDeTab(tabActiva)" :key="r.id_pregunta_base"
                    :class="['rounded-2xl border p-4', r.disparo_alerta ? 'border-red-300 bg-red-50' : 'border-border bg-surface']">
                    <div class="flex items-start gap-3">
                      <span :class="['w-6 h-6 rounded-lg text-xs font-black flex items-center justify-center shrink-0', r.disparo_alerta ? 'bg-red-500/15 text-red-600' : 'bg-accent/10 text-accent']">
                        {{ idx + 1 }}
                      </span>
                      <div class="min-w-0 flex-1">
                        <p class="text-sm font-semibold text-black leading-snug">{{ r.texto_pregunta }}</p>
                        <div class="mt-2 flex items-center gap-2 flex-wrap">
                          <span :class="['text-sm font-bold px-3 py-1 rounded-xl border', r.disparo_alerta ? 'bg-red-100 text-red-700 border-red-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200']">
                            {{ formatearRespuesta(r.tipo_control, r.valor_respuesta) }}
                          </span>
                          <span v-if="r.disparo_alerta" class="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-lg bg-red-500/10 text-red-600 border border-red-300">
                            <AlertTriangle class="w-3 h-3" /> Signo de alarma
                          </span>
                        </div>
                        <p v-if="r.tipo_control === 'opcion_multiple' && r.opciones?.length" class="text-[10px] text-muted mt-1.5">
