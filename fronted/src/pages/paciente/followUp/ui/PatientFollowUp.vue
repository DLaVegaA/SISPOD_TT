<template>
  <div class="fade-in max-w-2xl mx-auto pb-16">
    ...
    <div class="flex items-center gap-2 text-xs text-muted mb-6">
      <RouterLink
        :to="{ name: ROUTE_NAMES.PATIENT_HOME, params: routeParams }"
        class="text-muted hover:text-black transition-colors"
      >
        🏠
      </RouterLink>
      <span>/</span>
      <RouterLink
        :to="{ name: ROUTE_NAMES.PATIENT_FOLLOW_UP, params: routeParams }"
        class="text-muted hover:text-black transition-colors"
        @click="seguimientoSeleccionado = null"
      >
        Seguimiento
      </RouterLink>
      <template v-if="seguimientoSeleccionado">
        <span>/</span>
        <span class="font-medium text-black truncate max-w-[180px]">
    ...
    <div v-if="cargandoLista" class="flex items-center justify-center gap-3 py-24 text-muted">
      ...
      <span class="text-sm">Cargando tus seguimientos...</span>
    </div>

    <!-- ── Error de lista ─────────────────────────────────────────── -->
    <div
      v-else-if="errorLista"
      class="rounded-2xl border border-red-200 bg-red-50 p-5 flex items-start gap-3 text-sm text-red-700"
    >
    ...
    <div
      ...
      class="rounded-2xl border border-border bg-surface p-10 text-center"
    >
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <div class="flex items-center gap-2 shrink-0">
              <span :class="['text-xs font-medium px-2.5 py-1 rounded-full', estadoClase(s.estado_seguimiento)]">
    ...
    <!-- ══════════════════════════════════════════════════════════════ -->
    <!-- Vista B: detalle del seguimiento seleccionado                  -->
    <!-- ══════════════════════════════════════════════════════════════ -->
    <template v-else>

      <!-- Estado y procedimiento -->
      ...
      >
        <h2 class="text-sm font-semibold text-black flex items-center gap-2">
          <ClipboardList class="h-4 w-4 text-accent" />
      ...
      <div class="space-y-3">
        ...
        >
          <div class="flex items-center gap-3">
            <div :class="['w-8 h-8 rounded-lg flex items-center justify-center shrink-0', tipo.completado ? 'bg-green-100' : 'bg-surface border border-border']">
          ...
          <span :class="['text-xs font-medium px-2.5 py-1 rounded-full', tipo.completado ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700']">
    ...
    <Teleport to="body">
        ...
        >
          <div class="relative w-full sm:max-w-xl bg-white rounded-t-3xl sm:rounded-2xl shadow-xl flex flex-col max-h-[90dvh]">
            ...
            <div class="flex items-start justify-between p-5 border-b border-border shrink-0">
              ...
              <button class="p-1.5 rounded-xl hover:bg-surface text-muted transition-colors" aria-label="Cerrar" @click="form.intentarSalir()">
                <X class="h-5 w-5" />
              </button>
            </div>
            ...
            <div v-if="form.cargandoCuestionario.value" class="flex-1 flex items-center justify-center gap-3 text-muted p-10">
              <Loader2 class="h-6 w-6 animate-spin" />
              <span class="text-sm">Cargando preguntas...</span>
            </div>
            ...
            <div v-else class="overflow-y-auto flex-1 px-5 py-4 space-y-8">
              <div v-for="(estado, idx) in form.estados.value" :key="estado.pregunta.id_pregunta">
                ...
                <div class="pl-8">
                  <ControlPregunta
                    ...
                    @update:model-value="(v) => form.actualizarRespuesta(estado.pregunta.id_pregunta, v)"
                  />
                </div>
              </div>
            </div>
      ...
      <!-- Enviado con éxito -->
      <Transition name="fade">
        <div v-if="form.enviado.value && !form.mostrarAlerta.value" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" @click.self="cerrarModal">
          <div class="bg-white rounded-2xl p-8 max-w-sm w-full text-center space-y-4 shadow-xl">
            ...
            <button class="w-full rounded-xl bg-accent text-white text-sm font-semibold py-2.5 hover:bg-accent/90 transition-all" @click="cerrarModal">Entendido</button>
          </div>
        </div>
      </Transition>
      ...
      <!-- Alerta RN13 -->
      <Transition name="fade">
        <div v-if="form.mostrarAlerta.value" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div class="bg-white rounded-2xl p-8 max-w-sm w-full shadow-xl space-y-4">
            ...
            <button class="w-full rounded-xl bg-red-600 text-white text-sm font-semibold py-2.5 hover:bg-red-700 transition-all" @click="cerrarAlerta">Entendido</button>
          </div>
        </div>
      </Transition>
      ...
      <Transition name="fade">
        <div v-if="form.mostrarSalir.value" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 px-4">
          <div class="bg-white rounded-2xl p-6 max-w-xs w-full shadow-xl space-y-4">
            ...
            <div class="flex gap-3">
              <button class="flex-1 rounded-xl border border-border text-sm py-2.5 hover:bg-surface transition-colors" @click="form.cancelarSalir()">Continuar</button>
              <button class="flex-1 rounded-xl bg-black text-white text-sm py-2.5 hover:bg-black/80 transition-colors" @click="salirConfirmado">Salir</button>
            </div>
          </div>
        </div>
      </Transition>
...
function cerrarModal() {
  if (form.enviado.value) completados.value.add(tipoActivo.value)
